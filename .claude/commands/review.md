...
>
> **Repository context:** SvelteKit (Svelte 5 runes, TypeScript, adapter-node) with PostgreSQL (`pg`). This is the birds personal birding app (birds.gaylon.photos): eBird API integration, needs/targets computation, trips, and a link-out gallery to gaylon.photos. Focus on PostgreSQL type-boundary issues (JSONB-as-object, NUMERIC-as-string, TIMESTAMPTZ/UTC handling), async/await on database queries, eBird API error handling (rate limits, invalid keys, cache fallback — never log eBird credentials), the species-name→species_code matching logic, and Svelte 5 runes correctness ($state/$derived/$effect dependencies, server-vs-client load boundaries). Read the project's `cs.md` for additional context, especially the eBird credential and data integrity rules.
>
