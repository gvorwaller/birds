...
**Status:** BUILD SPEC rev 3.6 (2026-09-03). Rev 3.6: the phone readout sticks to the viewport bottom (CC1 Safari drive P2, in the Rev 3.4 ledger). Rev 3.5: two re-check P2s (unseed lock, phone hint copy) in the Rev 3.4 ledger. Rev 3.4: CODEX1's TD-C gate (REJECT at 5fbc0d7)
...
| P2 (Safari drive) | With 48 px rows the phone World chart is 884 px tall; the readout, ordered below the chart on phones, sat ~800 px under the viewport after a tap on an upper row (390×731: readout top 838), so "readout visible after a tap on a phone" failed. | Phone block: `.rlayout > .readout { position: sticky; bottom: calc(var(--nav-h) + env(safe-area-inset-bottom, 0px)); z-index: 2; background: var(--card) }` — the result stays pinned above the bottom nav while the chart scrolls. Oracle carries it, verified in Safari at 390. |
