...
| P2 (Safari drive) | With 48 px rows the phone World chart is 884 px tall; the readout, ordered below the chart on phones, sat ~800 px under the viewport after a tap on an upper row (390×731: readout top 838), so "readout visible after a tap on a phone" failed. | Phone block: `.rlayout > .readout { position: sticky; bottom: calc(var(--bottomnav-h) + env(safe-area-inset-bottom, 0px)); z-index: 2; background: var(--card) }` (`--bottomnav-h`, 64 px, is the fixed phone nav; `--nav-h`, 56 px, is the top nav — rev 3.6 first named the wrong one, caught by CODEX1's re-check) — the result stays pinned above the bottom nav while the chart scrolls. Oracle carries it, verified in Safari at 390. |
