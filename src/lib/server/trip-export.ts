...
function needsLabel(n: number): string {
	return `${n} of your needs reported here (last 14 days, ≤16 km)`;
}
...
export function buildTripMarkdown(data: TripExportData): string {
	...
	lines.push(`[Open in app](${origin}/trips/${trip.id})`, '');
	...
	stops.forEach((s, i) => {
		...
		if (n !== undefined) meta.push(needsLabel(n));
		...
		if (needed && needed.length > 0) {
			lines.push(
				'',
				`Needs: ` +
					needed.map((sp) => `[${sp.comName}](${speciesUrl(origin, sp.code, trip.id)})`).join(' · ')
			);
		}
	...
	lines.push('---', '');
	lines.push(
		`_Generated by birds.gaylon.photos on ${(data.generatedAt ?? new Date()).toLocaleDateString('en-US')}. Data from eBird.org._`
	);
	return lines.join('\n');
}
...
export function buildTripHtml(data: TripExportData): string {
	...
	parts.push(
		`<a class="openapp" href="${esc(`${origin}/trips/${trip.id}`)}">Open in app ↗</a>`
	);
	...
	stops.forEach((s, i) => {
		...
		const meta: string[] = [];
		const n = counts.get(s.id);
		if (n !== undefined) meta.push(esc(needsLabel(n)));
		if (s.lat != null && s.lon != null) {
	...
	parts.push(
		`<footer class="muted">Generated by birds.gaylon.photos on ${esc(
			(data.generatedAt ?? new Date()).toLocaleDateString('en-US')
		)}. Data from eBird.org.</footer>`
	);
