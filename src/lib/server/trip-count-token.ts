...
import {
  ...
  type AnyTripCountContext,
} from "$lib/trip-count-context";
...
export class TripCountTokenError extends Error {
  constructor(
    message = "Trip preview expired or changed — re-run the plan before saving.",
  ) {
    super(message);
...
function secret(): string {
  if (!env.AUTH_SECRET)
    throw new Error(
      "AUTH_SECRET is not set — cannot issue trip count snapshots",
    );
...
export function issueTripCountToken(
  ...
  if (
    ...
    accountId <= 0 ||
    !Number.isSafeInteger(scopeOwnerId) ||
    scopeOwnerId <= 0
  ) {
    throw new TripCountTokenError(
      "Cannot issue a trip count snapshot for this account.",
    );
  }
  const valid = parseAnyTripCountContext(context);
  if (!valid)
    throw new TripCountTokenError(
      "Cannot issue an invalid trip count snapshot.",
    );
  const plannedAt = Date.parse(valid.plannedAt);
  const expiresAt = plannedAt + 24 * 60 * 60 * 1000;
  const payload: TripCountTokenPayload = {
...
export function verifyTripCountToken(
  ...
  if (
    ...
    throw new TripCountTokenError();
  if (
    x.version !== TOKEN_VERSION ||
    x.accountId !== accountId ||
    x.scopeOwnerId !== scopeOwnerId
  )
    throw new TripCountTokenError();
  if (
    !Number.isSafeInteger(x.expiresAt) ||
    (x.expiresAt as number) <= now ||
    (x.expiresAt as number) !==
      Date.parse(
        String((x.context as Record<string, unknown> | null)?.plannedAt),
      ) +
        24 * 60 * 60 * 1000
  )
    throw new TripCountTokenError();
  const context = parseAnyTripCountContext(x.context);
  if (
    !context ||
    Date.parse(context.plannedAt) + 24 * 60 * 60 * 1000 !== x.expiresAt
  )
    throw new TripCountTokenError();
  return {
    version: 1,
    ...
    scopeOwnerId,
    context,
    expiresAt: x.expiresAt as number,
  };
}
