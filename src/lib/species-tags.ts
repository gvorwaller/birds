...
export function tagLabel(dimension: TagDimension, value: string): string {
  ...
  return dimension === "tide" && value !== "tide-independent"
    ? `Tide: ${text}`
    : text;
}

/**
 * Actionable tide stages carried by `tags` — 'tide:falling', 'tide:low', etc.
 * Excludes 'tide-independent' (a species that says tides don't matter is not
 * a candidate for a tide lookup) and anything outside `TAG_VOCABULARY.tide`.
 */
export function tideTagValues(tags: readonly string[]): string[] {
  return tags
    .filter((t) => t.startsWith("tide:"))
    .map((t) => t.slice("tide:".length))
    .filter(
      (v) =>
        v !== "tide-independent" &&
        (TAG_VOCABULARY.tide as readonly string[]).includes(v),
    );
}

/** True when this species has at least one actionable tide stage (td-6a3d2e). */
export function isTideTagged(tags: readonly string[]): boolean {
  return tideTagValues(tags).length > 0;
}
