...
<div class="page">
	<header class="page-head">
		<p class="sub"><a href="/targets">← Targets</a></p>
		<h1>
			{data.taxon.com_name}
			{#if data.seen}<Badge kind="seen" label="Seen" />{:else}<Badge kind="need" label="Need" />{/if}
		</h1>
		<p class="sub">
	...
	<section class="card">
		...
		{#if !data.hasApiKey || !data.hasHome}
			<p class="muted">
				Set your eBird API key and home location in <a href="/settings">Settings</a> to see nearby reports.
			</p>
	...
	<section class="card">
		...
		{#each links as l (l.href)}
			<div class="obs">
				<div class="grow">
					<div class="name"><a href={l.href} target="_blank" rel="noopener">{l.label}</a></div>
	...
	<p class="attribution">
		Data from <a href="https://ebird.org" target="_blank" rel="noopener">eBird.org</a> · photos on
		<a href="https://gaylon.photos/birds" target="_blank" rel="noopener">gaylon.photos</a>
	</p>
</div>
