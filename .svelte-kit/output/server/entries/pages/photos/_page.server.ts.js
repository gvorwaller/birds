...
import { b as buildMatcher, n as normalizeName, s as syncGallery, r as refreshGalleryIfStale, a as allPhotoLinks } from "../../../chunks/gallery.js";
const load = async () => {
...
const actions = {
  refresh: async () => {
    try {
      ...
      return { ok: true, message: `Synced ${result.total} photos (${result.matched} matched).` };
    } catch (err) {
      return fail(502, {
  ...
  override: async ({ request }) => {
    ...
    return { ok: true, message: `Matched "${sourceName}" → ${code}.` };
  }
};
export {
  actions,
