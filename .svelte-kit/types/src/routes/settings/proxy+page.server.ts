...
export const actions = {
	save_api_key: async ({ locals, request }: import('./$types').RequestEvent) => {
		const userId = locals.user!.id;
	...
	save_login: async ({ locals, request }: import('./$types').RequestEvent) => {
		const userId = locals.user!.id;
		const form = await request.formData();
	...
	save_home: async ({ locals, request }: import('./$types').RequestEvent) => {
		const userId = locals.user!.id;
		const form = await request.formData();
	...
	sync_taxonomy: async ({ locals }: import('./$types').RequestEvent) => {
		const userId = locals.user!.id;
	...
	sync_lifelist: async ({ locals }: import('./$types').RequestEvent) => {
		const userId = locals.user!.id;
		try {
	...
	import_csv: async ({ locals, request }: import('./$types').RequestEvent) => {
		const userId = locals.user!.id;
		const form = await request.formData();
